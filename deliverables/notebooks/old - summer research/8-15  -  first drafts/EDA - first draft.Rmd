...
First, let's define the word "Robotic" to refer to songs where an external songwriter was credited.

We first read in our data. Each row is a song on Billboard and each column is an attribute. We remove songs that appear > 1 time on Billboard and we keep the observation with the maximum amount of weeks on the charts.
...
write.csv(final_df, file = paste0(desktop, "\\", "Weekly_data_top_week.csv"))
...
Let's dive deeper into the lyrical content. We create a document term matrix where we filter out terms that appear in less than 10 songs. We then run PCA on it. Below is a visualization of the first two principal components:
...
We can see that between robotic and non-robotic songs, the principal components do not separate our data very well.
...
Since PCA did not separate the groups very well, we did not expect K-means to find good clusters to describe robotic vs. non-robotic songs. However, for K-means, we can see the algorithm was able to find two clusters: one with a large variance and the other shaped more like an ellipse.
