...
each_song <- df_words_unique %>% mutate(
        ...
        word = gsub("([A-z])'[A-z]+", '\\1', word), #removes contractions. Debatable.
        word = gsub("[^A-z]", '', word),
        word = gsub("\\s+", "", word)
      ) %>%
    mutate(
...
song_complexity <- each_song %>%  mutate(
  ...
  is_profanity = str_detect(word, regex("fuck|shit|bitch|nigga", ignore_case = TRUE))#word %in% c("fuck", "shit", " bitch", " nigga")
  ) %>% group_by(Name, Artists, Songwriter) %>% 
  summarize(
     Total_Words = sum(n),
     ...
     tot_profanity = sum(ifelse(is_profanity == TRUE, n, 0)),
     profanity_ratio = tot_profanity / Total_Words
) %>%
